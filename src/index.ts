import {app, BrowserWindow, screen, Tray, Menu, nativeImage, ipcMain, Display} from 'electron';
...
async function createWindowForDisplay(display: Display): Promise<BrowserWindow> {
    ...
    ipcMain.on('mouse-move', (event, data: { ignore: boolean }) => {
        newWindow.setIgnoreMouseEvents(data.ignore, {forward: true});
    });
